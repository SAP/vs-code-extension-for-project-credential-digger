 ...
 * @param aiPrompts - The AI prompts and documentation. Will be loaded from a JSON file.
 * @param languages - The languages options. Will be loaded from a JSON file.
...
export class QuickFixProviderEnvVar implements CodeActionProvider {
    ...
    private aiPrompts: AiPromptsAndDocs | null = null;
    ...
    async executeCodeAction(
        ...
        try {
            // load the languages options from a JSON file. True means it is a language file.
            this.languages = await loadJsonData(
                '../src/lib/code-actions/languages.json',
                true,
            );
            // load the AI prompts and documentation from a JSON file. False means it is not a language file.
            this.aiPrompts = await loadJsonData(
    ...
    getOptions(extension: keyof Languages) {
        ...
        const allOptions = this.languages[extension];
    ...
    async callAI(
        ...
        try {
            const jsonPrompt = this.getAiPromptsAndDocsValue('promt_env_var'); // get the ai prompt for creating environment variables

            if (typeof jsonPrompt !== 'string') {
                throw new Error('promt_env_var should be a string');
            }

            const prompt = jsonPrompt.replace('EXTENSION_TMP', extension);
            const response = await getAIResponse(prompt);

            if (response.success === true) {
                // insert the instructions from the AI to the document
                edit.insert(uri, diagnostic.range.end, response.message);
            } else {
    ...
    /**
     * Get the prompt from the aiPromptAndDocumentation json file.
     *
     * @param key - The key matching to the needed prompt from the AI prompts and documentation.
     */
    getAiPromptsAndDocsValue(key: keyof AiPromptsAndDocs) {
        if (!this.aiPrompts) {
            throw new Error('JSON data not loaded');
        }
        return this.aiPrompts[key];
    }
